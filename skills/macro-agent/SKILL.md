...
2. **LOOK** for sequences that match the user's intent (e.g., `example_send_message` for "send message")
...
2. seq-run example_send_message     # Run a base messaging sequence
...
The repository includes generic example sequences.
- `example_open_browser` - Opens a browser icon and waits
- `example_send_message` - Base flow for message sending (generic)
- Other user-created local sequences may exist - always run `seq-list` first!
...
- **Local runtime data**: `data/local/` (captures, sequences, sound state)
- **Safe examples (repo)**: `data/examples/` (template definitions and sample sequences)
...
This command proxies to the dedicated `region-capture` skill and writes to shared runtime data.
...
# 2. Found a matching base sequence
